...
[ -z "$STACK_TYPE" ] && STACK_TYPE="Unknown" && STACK_FINGERPRINT="Unknown stack"

PROJECT_STRUCTURE=$(find "$PWD" -maxdepth 2 -type d \
    ! -name ".*" ! -name "__pycache__" ! -name "node_modules" \
    ! -name ".venv" ! -name "venv" ! -name "dist" ! -name "build" \
    | sort | sed "s|$PWD/||" | grep -v '^$')
...
| Task Type                                      | Model                        |
|------------------------------------------------|------------------------------|
| File editing, terminal commands, routine code  | gemini-3-flash               |
| Debugging, intermediate reasoning              | gemini-3.1-pro-low           |
| Architecture design, complex problem-solving   | gemini-3.1-pro-high          |
...
| Second opinion / fallback                      | gpt-oss-120b-medium          |
EOF
...
NEW_FS_ENTRY=$(jq -n --arg pwd "$PWD" '{
    "command": "npx",
    "args": ["-y", "@modelcontextprotocol/server-filesystem", $pwd]
}')

NEW_GIT_ENTRY=$(jq -n --arg pwd "$PWD" '{
    "command": "npx",
    "args": ["-y", "@modelcontextprotocol/server-git", "--repository", $pwd]
}')

if [ -f "$MCP_CONFIG" ]; then
    # Non-destructive merge: inject our keys, keep everything else
    UPDATED=$(jq \
        --argjson fs "$NEW_FS_ENTRY" \
        --argjson git "$NEW_GIT_ENTRY" \
        '.mcpServers["local-filesystem"] = $fs | .mcpServers["local-git"] = $git' \
        "$MCP_CONFIG")
    echo "$UPDATED" > "$MCP_CONFIG"
    echo "✅ MCP merged into existing $MCP_CONFIG"
else
    jq -n \
        --argjson fs "$NEW_FS_ENTRY" \
        --argjson git "$NEW_GIT_ENTRY" \
        '{"mcpServers": {"local-filesystem": $fs, "local-git": $git}}' \
        > "$MCP_CONFIG"
