...
final_output() {
    ...
    echo -e "\n"
    echo -e "${GREEN}####################################################${NC}"
    echo -e "${GREEN}#        INFISICAL INSTALLED SUCCESSFULLY          #${NC}"
    echo -e "${GREEN}####################################################${NC}"
    echo -e "\n${YELLOW}--- IMPORTANT: INITIAL CONFIGURATION ---${NC}"
    echo -e "The master administrator account must be created during"
    echo -e "the first access via the web interface."
    echo -e "\n${BLUE}Access Details:${NC}"
    echo -e "URL: ${SITE_URL}"
    echo -e "Internal Access: http://${IP_ADDRESS}:8080"
    echo -e "\n${YELLOW}Generated Credentials (Keep them safe in $CONF_FILE!):${NC}"
    echo -e "ENCRYPTION_KEY: ${ENCRYPTION_KEY}"
    echo -e "AUTH_SECRET: ${AUTH_SECRET}"
    echo -e "\n${BLUE}Service Status:${NC}"
    infisical-ctl status
    echo -e "\n${BLUE}Monitor logs by running:${NC} infisical-ctl tail"
    echo -e "\n${YELLOW}Application Integration (CLI):${NC}"
    echo -e "Read the CLI documentation: https://infisical.com/docs/cli/overview"
    echo -e "${GREEN}####################################################${NC}\n"
}
